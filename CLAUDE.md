...
- **desktop** — base + Niri/DankMaterialShell and COSMIC, both installed side by side and switchable from the graphical dms-greeter, plus a bluefin-dx-inspired developer experience layer (Homebrew, Docker, VS Code, mise, Ptyxis, Flatpak). Boots to the login screen.
...
2. **The two flavors use greeters appropriate to their scope.** Server keeps
   ...
   session. Desktop uses `dms-greeter` (AvengeMedia's DMS-themed greetd
   frontend), chosen over the COSMIC package set's own `cosmic-greeter`
   because it remembers the last user/session *and* syncs DMS's wallpaper
   and matugen theme into the login screen — the second deliberate
   third-party-repo exception in this project (a Copr, `avengemedia/danklinux`,
   alongside Docker's official repo below) since it isn't in Fedora's own
   repos. `/usr/share/wayland-sessions/*.desktop` entries are written
   explicitly for both desktop sessions.
...
desktop/Containerfile                # FROM base; niri/dms + cosmic + dev-experience layer
...
`FROM` base, six sections in this order. The order is cache-conscious, not just narrative: every package-installing step (Desktops, dms-greeter, Developer experience) is grouped up front, and the cheap, frequently-edited config/dotfiles/theming steps (Session entries, Per-DE wiring, Login/session switching) come after — Docker/buildx layer caching is positional, so an edit to one instruction invalidates every layer after it in the file, and this image's theming/dotfiles config churns far more than its package list.

1. **Desktops** — one `dnf install` for both DEs' packages plus shared portal/audio/font plumbing. `cosmic-desktop` is a comps group (`dnf group install`) and pulls in `cosmic-greeter`. Package names/versions were confirmed directly against live Fedora 44 repos while building this image (`dnf5 repoquery`), not assumed — same verification discipline the reference repo used. No COPRs for anything in this section.
2. **dms-greeter** — installed here rather than down with its runtime wiring, since its Copr enable/install/disable is expensive and belongs with the other package installs, not the cheap config sections below. Provides a graphical session chooser for both sessions, reading the same `/usr/share/wayland-sessions` entries as any other greeter regardless of the niri compositor it renders itself with. Installed from its own Copr (`avengemedia/danklinux`) since it isn't in Fedora's repos.
3. **Developer experience** — bluefin-dx-inspired, not a 1:1 port (skips Incus, JetBrains Toolbox, GPU compute libs, kernel tracing tools — none of that was asked for; add it the same way if it's ever wanted). Only the package/binary installs live here; each tool's cheap per-user config/theming wiring lives down in Per-DE wiring instead:
   - **Docker Engine** via Docker's official repo (`download.docker.com`) — Fedora's own repos don't ship `docker-ce`, so this is one of two deliberate exceptions to the "Fedora repos only, no third-party repos" rule the rest of this image family follows (the other is `dms-greeter`'s Copr, above). Uses dnf5's `config-manager addrepo --from-repofile=` syntax, not dnf4's `--add-repo`.
   ...
   - **VS Code** via Microsoft's official repo; its first-login `vscode-dms-theme.service` (same one-time pattern as the chezmoi dotfiles init) that installs DMS's bundled dynamic-theme extension and a `create_`-seeded `Code/User/settings.json` selecting it lives down in Per-DE wiring, not here.
   ...
   - **Flatpak** itself (`dnf install flatpak`) only — the actual Flathub remote add and app installs (Zen, Flatseal, Bazaar), deferred to a first-boot `flatpak-bootstrap.service` since both write under `/var/lib/flatpak` (which bootc doesn't re-seed on `bootc update` of an already-provisioned machine, only on a genuinely fresh deployment), live down in Per-DE wiring.
4. **Session entries** — explicit `/usr/share/wayland-sessions/{niri,cosmic}.desktop`, so the graphical greeter always shows these two stable names and commands.
5. **Per-DE wiring** — the original collection under `wallpapers/` is installed to `/usr/share/backgrounds/bootsy-linux` for every picker, with `bootsy-zircon-flow.webp` as the common default. Niri's chezmoi source under `/usr/share/bootsy/dotfiles` is derived from niri's own shipped default (Waybar autostart stripped, Fuzzel binding replaced by DMS Spotlight, focus-ring recolored from niri's stock light blue to a restrained neutral gray-blue, `prefer-no-csd` and a `geometry-corner-radius`/`clip-to-geometry` window-rule turned on so the focus-ring/shadow actually follow rounded window corners instead of framing the client's full rectangular surface, shadows turned on at niri's own stock softness/spread/offset/color, DMS and swaybg spawned at startup) and shared systemd user units install it to `~/.config/niri/config.kdl` before the first graphical session and track image updates; `~/.config/niri/local.kdl` is reserved for durable user overrides. The same chezmoi source also seeds `~/.local/state/DankMaterialShell/session.json` with `wallpaperPath` (via chezmoi's `create_` prefix — written once, never reapplied) so DMS's own background layer, which is otherwise blank until configured through its settings UI, agrees with swaybg's wallpaper from first login instead of briefly showing nothing once DMS finishes starting. It likewise `create_`-seeds `~/.config/DankMaterialShell/settings.json` with `currentThemeName: "dynamic"`, switching DMS from its static default theme to matugen-driven wallpaper theming — every relevant per-app template toggle already defaults on in DMS itself — and niri's `config.kdl` includes `dms/colors.kdl`, the file DMS's built-in niri matugen template writes to, so the focus-ring accent color updates with the wallpaper (falling back to the static neutral color above until that file exists). GTK and alacritty get the same treatment: `~/.config/gtk-{3,4}.0/gtk.css` (always-managed, pure plumbing) `@import`s DMS's generated `dank-colors.css`, and `~/.config/alacritty/alacritty.toml` (`create_`-seeded, user-owned) imports DMS's generated `dank-theme.toml`; neither GTK nor alacritty loads those generated filenames on its own. cosmic's wallpaper uses an `/etc/xdg/cosmic/...` override and has its own separate (static) theming system. VS Code's and Flatpak's cheap per-user/runtime wiring (see Developer experience above) also live here.
6. **Login / session switching** — `dms-greeter`'s cheap runtime wiring (the package itself was installed earlier, see above): `cosmic-greeter`, still pulled in by the `cosmic-desktop` group, is explicitly disabled in favor of it, and greetd's own generic unit is enabled and pointed at `dms-greeter` via a hand-written `config.toml`.
7. `systemctl set-default graphical.target` — unlike `server`, `desktop` boots straight to the login screen.
